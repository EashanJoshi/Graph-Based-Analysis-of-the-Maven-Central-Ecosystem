...
// Main QUERY - 2005 Results


CALL apoc.export.csv.query(
  "MATCH (a:Artifact)-[:relationship_AR]->(release:Release)
   WHERE datetime({epochMillis: release.timestamp}).year = 2005  // Filter releases for the year 2006
   ...
   WHERE datetime({epochMillis: release.timestamp}).year = 2005  // Ensure release year filter for versions
  ...
  "file:///all_results_2005_detailed.csv",  // Custom path
  {batchSize: 1000}
)
